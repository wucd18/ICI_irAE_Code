...
theme_set(theme_classic(base_size=7.7,base_family='sans')+theme(text=element_text(colour=ink),axis.text=element_text(colour=ink,size=7.5),axis.title=element_text(size=7.5),axis.line=element_line(linewidth=.18),axis.ticks=element_line(linewidth=.18),plot.title=element_text(size=8.5,face='bold'),plot.subtitle=element_text(size=7.5),legend.title=element_text(size=7.5),legend.text=element_text(size=7.5),legend.key.size=unit(2.5,'mm'),legend.margin=margin(0,0,0,0),strip.background=element_blank(),strip.text=element_text(size=7,face='bold'),plot.margin=margin(5,8,5,10),legend.spacing.x=unit(2,'mm'),legend.key.width=unit(4,'mm'),panel.spacing.x=unit(5,'mm')))
...
pathlab<-function(x){z<-gsub('_',' ',sub('HALLMARK_','',x));z<-tools::toTitleCase(tolower(z));z<-gsub('Interferon Alpha Response','IFN-alpha response',z);z<-gsub('Interferon Gamma Response','IFN-gamma response',z);z<-gsub('Tnfa Signaling Via Nfkb','TNF / NF-\u03baB',z,ignore.case=TRUE);z<-gsub('Epithelial Mesenchymal Transition','Epithelial-mesenchymal transition',z);z<-gsub('Il6 Jak Stat3 Signaling','IL6 / JAK / STAT3',z);z<-gsub('Il2 Stat5 Signaling','IL2 / STAT5',z);z<-gsub('G2m','G2M',z);z<-gsub('Myc','MYC',z);z<-gsub('Dna','DNA',z);z<-gsub('Uv','UV',z);z<-gsub('Mtorc1','mTORC1',z);z}
